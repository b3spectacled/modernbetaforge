...
public class BiomeDecoratorBeta extends ModernBetaBiomeDecorator {
    ...
    public void decorate(World world, Random random, Biome biome, BlockPos startPos) {
        ...
        if (TerrainGen.decorate(world, random, chunkPos, DecorateBiomeEvent.Decorate.EventType.FLOWERS)) {
            ...
            repeater.repeat(() -> populateWorldGenCount(world, random, startPos, FEATURE_DANDELION, mutablePos, plantCount, settings.height, settings.floor));
        }
        ...
        if (settings.useTallGrass && TerrainGen.decorate(world, random, chunkPos, DecorateBiomeEvent.Decorate.EventType.GRASS)) {
            ...
            repeater.repeat(() -> populateTallGrassCount(world, random, biome, startPos, mutablePos, plantCount, settings.height, settings.floor));
        }
        ...
        if (settings.useTallGrass && TerrainGen.decorate(world, random, chunkPos, DecorateBiomeEvent.Decorate.EventType.DEAD_BUSH)) {
            if (biome instanceof BiomeBetaDesert) {
                repeater.repeat(() -> populateWorldGenCount(world, random, startPos, FEATURE_DEAD_BUSH, mutablePos, 2, settings.height, settings.floor));
            }
        }
        
        if (TerrainGen.decorate(world, random, chunkPos, DecorateBiomeEvent.Decorate.EventType.FLOWERS)) {
            repeater.repeat(() -> populateWorldGenChance(world, random, startPos, FEATURE_POPPY, mutablePos, 2, settings.height, settings.floor));
        }
        
        if (TerrainGen.decorate(world, random, chunkPos, DecorateBiomeEvent.Decorate.EventType.SHROOM)) {
            repeater.repeat(() -> populateWorldGenChance(world, random, startPos, FEATURE_BROWN_SHROOM, mutablePos, 4, settings.height, settings.floor));
            repeater.repeat(() -> populateWorldGenChance(world, random, startPos, FEATURE_RED_SHROOM, mutablePos, 8, settings.height, settings.floor));
        }
        
        if (TerrainGen.decorate(world, random, chunkPos, DecorateBiomeEvent.Decorate.EventType.REED)) {
            repeater.repeat(() ->populateWorldGenCount(world, random, startPos, FEATURE_REED, mutablePos, 10, settings.height, settings.floor));
        }
        
        if (TerrainGen.decorate(world, random, chunkPos, DecorateBiomeEvent.Decorate.EventType.PUMPKIN)) {
            repeater.repeat(() -> populateWorldGenChance(world, random, startPos, FEATURE_PUMPKIN, mutablePos, 32, settings.height, settings.floor));
        }
        ...
        if (biome instanceof BiomeBetaDesert && TerrainGen.decorate(world, random, chunkPos, DecorateBiomeEvent.Decorate.EventType.CACTUS)) {
            repeater.repeat(() -> populateWorldGenCount(world, random, startPos, FEATURE_CACTUS, mutablePos, 10, settings.height, settings.floor));
        }
