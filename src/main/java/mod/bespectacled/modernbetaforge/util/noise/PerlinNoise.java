...
public class PerlinNoise {
        ...
        for (int sX = 0; sX < sizeX; sX++) {
            for (int sZ = 0; sZ < sizeZ; sZ++) {
                for (int sY = 0; sY < sizeY; sY++) {
                    ...
                    double res = MathUtil.lerp(w, MathUtil.lerp(v, lerp0, lerp1), MathUtil.lerp(v, lerp2, lerp3));
    ...
    private double sampleXYZ(int floorX, int floorY, int floorZ, double localX, double localOffsetY, double localZ, double localY) {
        ...
        return MathUtil.lerp(
            ...
            MathUtil.lerp(
                v, 
                MathUtil.lerp(u, grad0, grad1),
                MathUtil.lerp(u, grad2, grad3)
            ),
            MathUtil.lerp(
                v,
                MathUtil.lerp(u, grad4, grad5),
                MathUtil.lerp(u, grad6, grad7)
            )
        );
    }
